...
class ChatService:
    ...
    async def _handle_rag_stream(self, query: str, user_id: str,
        ...
        try:
            ...
            yield f"data: {json.dumps({'event': 'error', 'data': f'检索失败: {str(e)}'})}\n\n"
            return
        ...
        if not documents:
            yield f"data: {json.dumps({'event': 'token', 'data': '知识库中未找到相关内容。'})}\n\n"
        elif answer:
            # 流式输出 LLM 生成的回答
            try:
        ...
        # 持久化
        self._memory.append_messages(session_id, query, answer or "未找到相关内容")
