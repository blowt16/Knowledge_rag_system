...
def _dedup_pages(page_hashes: dict[int, str]) -> dict[int, int]:
    ...
    for page_num, phash in page_hashes.items():
        ...
        for rep_page, (rep_hash, members) in groups.items():
            try:
                dist = current_hash - imagehash.hex_to_hash(rep_hash)
                if dist <= max_dist:
...
async def process_mix_pdf(pdf_path: str, user_id: str, pdf_md5: str,
    ...
    try:
        ...
        for page_num in range(1, len(doc) + 1):
            ...
            content = f"{text}\n\n[页面视觉描述]: {vision_descriptions[vision_key]}" if vision_key in vision_descriptions else text
            if content.strip():
                blocks.append({
                    "page_num": page_num, "block_type": "mix", "content": content,
...
async def process_scan_pdf(pdf_path: str, user_id: str, pdf_md5: str,
    ...
    try:
        ...
        page_descriptions = {}
        for i in range(0, len(list(unique_pages)), batch_size):
            batch = sorted(unique_pages)[i:i + batch_size]
            tasks = []
            temp_paths = []
        ...
        for page_num in range(1, len(doc) + 1):
            ...
            blocks.append({
                "page_num": page_num, "block_type": "scan",
